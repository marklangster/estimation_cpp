...
//  if (ekfState(6) < -F_PI) ekfState(6) += 2.f*F_PI;
    ...
    // Get the quaternion from the Eurler angles
    Quaternion<float> attitudeQuaternion = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
    
    // Integrate the body rate given the gyro vector and our time constant
    attitudeQuaternion.IntegrateBodyRate(gyro, dtIMU);

    // Set our predicted values
    float predictedPitch = attitudeQuaternion.Pitch();
    float predictedRoll = attitudeQuaternion.Roll();
    
    // Set our yaw and normalize as was given in the sample code
    ekfState(6) = attitudeQuaternion.Yaw();
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    // Get our inertial frame acceleration vector
    V3F inertialFrameAccel = attitude.Rotate_BtoI(accel);

    // Implementation of the transition function in 7.2 (49)
    predictedState(0) = (dt * curState(3)) + curState(0);
    predictedState(1) = (dt * curState(4)) + curState(1);
    predictedState(2) = (dt * curState(5)) + curState(2);
    predictedState(3) = (dt * inertialFrameAccel.x) + curState(3);
    predictedState(4) = (dt * inertialFrameAccel.y) + curState(4);
    predictedState(5) = (dt * (inertialFrameAccel.z - CONST_GRAVITY)) + curState(5);
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    // Implementation of Equation 71 from Diebel in 7.2 (52)
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    // Implementation of the Jacobian g prime from 7.2 (50)
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    // H prime just the identity matrix
    for ( int i = 0; i < 6; i++)
    {
        hPrime(i,i) = 1;
    }
    
    // Set based on 7.3.1 (53)
    zFromX(0) = ekfState(0);
    zFromX(1) = ekfState(1);
    zFromX(2) = ekfState(2);
    zFromX(3) = ekfState(3);
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    // find the difference and define our h prime
    float normalizedYaw = magYaw - ekfState(6);
    hPrime(0, 6) = 1;
    
    // Set our yaw and normalize with a modified version of the sample code
    if (normalizedYaw > F_PI) zFromX(0) = ekfState(6) - 2.f*F_PI;
    else if (normalizedYaw < -F_PI) zFromX(0) = ekfState(6) + 2.f*F_PI;
    else zFromX(0) = ekfState(6);
